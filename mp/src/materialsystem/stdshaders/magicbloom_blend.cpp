...
#include "magicbloom_blend_ps30.inc"

ConVar mat_magicbloom_threshold("mat_magicbloom_threshold", "2.0", FCVAR_ARCHIVE, "Thresholds (limits) dark pixels from being accounted for bloom.");
ConVar mat_magicbloom_intensity("mat_magicbloom_intensity", "1.0", FCVAR_ARCHIVE, "Amount of bloom applied to the image.");
ConVar mat_magicbloom_dirt_intensity("mat_magicbloom_dirt_intensity", "0.0", FCVAR_ARCHIVE, "Amount of lens dirt applied to bloom.");
...
SHADER_DRAW
{
	...
	DYNAMIC_STATE
	{
		...
		float threshold = mat_magicbloom_threshold.GetFloat();
		float intensity = mat_magicbloom_intensity.GetFloat();
		float dirt_intensity = mat_magicbloom_intensity.GetFloat();

		pShaderAPI->SetPixelShaderConstant(0, &threshold);
		pShaderAPI->SetPixelShaderConstant(1, &intensity);
		pShaderAPI->SetPixelShaderConstant(2, &dirt_intensity);
