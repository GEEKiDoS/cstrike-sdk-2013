...
public:
	CConceptInfoMap() :
	  CUtlMap<AIConcept_t, ConceptInfo_t *>( CaselessStringLessThan )
	  {
		  ...
		  {
			  Insert( g_ConceptInfos[i].koncept, &g_ConceptInfos[i] );
		  }
	  }
};
...
void CAI_AllySpeechManager::Spawn()
{
	...
	for ( int i = 0; i < ARRAYSIZE(g_ConceptInfos); i++ )
		m_ConceptTimers.Insert( AllocPooledString( g_ConceptInfos[i].koncept ), CSimpleSimTimer() );
}
...
void CAI_AllySpeechManager::AddCustomConcept( const ConceptInfo_t &conceptInfo )
{
	...
	if ( g_ConceptInfoMap.Find( conceptInfo.koncept ) == g_ConceptInfoMap.InvalidIndex() )
	{
		g_ConceptInfoMap.Insert( conceptInfo.koncept, new ConceptInfo_t( conceptInfo ) );
	}

	if ( m_ConceptTimers.Find( AllocPooledString(conceptInfo.koncept) ) == m_ConceptTimers.InvalidIndex() )
	{
		m_ConceptTimers.Insert( AllocPooledString( conceptInfo.koncept ), CSimpleSimTimer() );
	}
}
...
ConceptInfo_t *CAI_AllySpeechManager::GetConceptInfo( AIConcept_t koncept )
{
	int iResult = g_ConceptInfoMap.Find( koncept );

	return ( iResult != g_ConceptInfoMap.InvalidIndex() ) ? g_ConceptInfoMap[iResult] : NULL;
}

void CAI_AllySpeechManager::OnSpokeConcept( CAI_PlayerAlly *pPlayerAlly, AIConcept_t koncept, AI_Response *response )
{
	ConceptInfo_t *			pConceptInfo	= GetConceptInfo(koncept);
		...
		if ( pConceptInfo->flags & AICF_PROPAGATE_SPOKEN ) 
		{
			...
			for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
			{
					 ...
					 pPlayerAlly->FVisible( pTalker ) )
				{
					...
					pTalker->GetExpresser()->SetSpokeConcept(koncept, NULL, false );
				}
			}
		}
	}
	...
	if ( !ai_no_talk_delay.GetBool() )
	{
		...
		if ( pConceptInfo && pConceptInfo->minConceptDelay != -1 )
		{
			...
			char iConceptTimer = m_ConceptTimers.Find( MAKE_STRING(koncept) );
...
bool CAI_AllySpeechManager::ConceptDelayExpired( AIConcept_t koncept )
{
	char iConceptTimer = m_ConceptTimers.Find( MAKE_STRING(koncept) );
	if ( iConceptTimer != m_ConceptTimers.InvalidIndex() )
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SelectSpeechResponse( AIConcept_t koncept, const char *pszModifiers, CBaseEntity *pTarget, AISpeechSelection_t *pSelection )
{
	if ( IsAllowedToSpeak( koncept ) )
	{
		bool result = SpeakFindResponse( pSelection->Response, koncept, pszModifiers );
		if ( result )
		{
			pSelection->koncept = koncept;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::SetPendingSpeech( AIConcept_t koncept, AI_Response &Response )
{
	m_PendingResponse = Response;
	m_PendingConcept = koncept;
...
bool CAI_PlayerAlly::SelectInterjection()
{
	...
	if ( GetState() == NPC_STATE_IDLE || GetState() == NPC_STATE_ALERT )
	{
		...
		{
			SetSpeechTarget( selection.hSpeechTarget );
			SpeakDispatchResponse( selection.koncept.c_str(), selection.Response );
			return true;
		}
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::PostSpeakDispatchResponse( AIConcept_t koncept, AI_Response *response )
{
#ifdef HL2_EPISODIC
...
int CAI_PlayerAlly::SelectNonCombatSpeechSchedule()
{
	if ( !HasPendingSpeech() )
	{
		...
		{
			SetSpeechTarget( selection.hSpeechTarget );
			SetPendingSpeech( selection.koncept.c_str(), selection.Response );
		}
	}
...
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::ShouldSpeakRandom( AIConcept_t koncept, int iChance )
{
	CAI_AllySpeechManager *	pSpeechManager	= GetAllySpeechManager();
	ConceptInfo_t *			pInfo			= pSpeechManager->GetConceptInfo( koncept );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::IsAllowedToSpeak( AIConcept_t koncept, bool bRespondingToPlayer ) 
{ 
	CAI_AllySpeechManager *	pSpeechManager	= GetAllySpeechManager();
	ConceptInfo_t *			pInfo			= pSpeechManager->GetConceptInfo( koncept );
	ConceptCategory_t		category		= ( pInfo ) ? pInfo->category : SPEECH_IDLE;
	...
	{
		if ( CompareConcepts( koncept, TLK_HELLO ) )
			return false;
		if ( CompareConcepts( koncept, TLK_HELLO_NPC ) )
			return false;
	}
			
	if ( !pSpeechManager->ConceptDelayExpired( koncept ) )
		return false;

	if ( ( pInfo && pInfo->flags & AICF_SPEAK_ONCE ) && GetExpresser()->SpokeConcept( koncept ) )
		return false;

	if ( !GetExpresser()->CanSpeakConcept( koncept ) )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_PlayerAlly::SpeakIfAllowed( AIConcept_t koncept, const char *modifiers, bool bRespondingToPlayer, char *pszOutResponseChosen, size_t bufsize ) 
{ 
	if ( IsAllowedToSpeak( koncept, bRespondingToPlayer ) )
	{
		return Speak( koncept, modifiers, pszOutResponseChosen, bufsize );
	}
	return false;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_PlayerAlly::OnSpokeConcept( AIConcept_t koncept, AI_Response *response )
{
	CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
	pSpeechManager->OnSpokeConcept( this, koncept, response );
