...
// Output : AI_Response
//-----------------------------------------------------------------------------
bool CAI_Expresser::SpeakFindResponse( AI_Response &outResponse, AIConcept_t koncept, const char *modifiers /*= NULL*/ )
{
	...
	set.AppendCriteria( "concept", koncept, CONCEPT_WEIGHT );
...
// Input  : *response - 
//-----------------------------------------------------------------------------
bool CAI_Expresser::SpeakDispatchResponse( AIConcept_t koncept, AI_Response& response, IRecipientFilter *filter /* = NULL */ )
{
	bool spoke = false;
	...
	if ( IsSpeaking() && koncept[0] != 0 )
	{
		DevMsg( "SpeakDispatchResponse:  Entity ( %i/%s ) already speaking, forcing '%s'\n", GetOuter()->entindex(), STRING( GetOuter()->GetEntityName() ), koncept );
	...
	if ( spoke )
	{
		...
		if ( DebuggingSpeech() && g_pDeveloper->GetInt() > 0 && response.GetType() != RESPONSE_PRINT )
		{
			Vector vPrintPos;
			GetOuter()->CollisionProp()->NormalizedToWorldSpace( Vector(0.5,0.5,1.0f), &vPrintPos );
			NDebugOverlay::Text( vPrintPos, CFmtStr( "%s: %s", koncept, szResponse ), true, 1.5 );
		}
		...
		SetSpokeConcept( koncept, &response );
	}
...
//-----------------------------------------------------------------------------
bool CAI_Expresser::Speak( AIConcept_t koncept, const char *modifiers /*= NULL*/, char *pszOutResponseChosen /* = NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ )
{
    AI_Response response;
	bool result = SpeakFindResponse( response, koncept, modifiers );
	if ( !result )
		return false;

	SpeechMsg( GetOuter(), "%s (%p) spoke %s (%f)\n", STRING(GetOuter()->GetEntityName()), GetOuter(), koncept, gpGlobals->curtime );

	bool spoke = SpeakDispatchResponse( koncept, response, filter );
...
//-------------------------------------
bool CAI_Expresser::CanSpeakConcept( AIConcept_t koncept )
{
	// Not in history?
	int iter = m_ConceptHistories.Find( koncept );
...
bool CAI_Expresser::SpokeConcept( AIConcept_t koncept )
{
	return GetTimeSpokeConcept( koncept ) != -1.f;
}

//-------------------------------------

float CAI_Expresser::GetTimeSpokeConcept( AIConcept_t koncept )
{
	int iter = m_ConceptHistories.Find( koncept );
	if ( iter == m_ConceptHistories.InvalidIndex() ) 
		return -1;
...
void CAI_Expresser::SetSpokeConcept( AIConcept_t koncept, AI_Response *response, bool bCallback )
{
	int idx = m_ConceptHistories.Find( koncept );
	if ( idx == m_ConceptHistories.InvalidIndex() )
	{
		ConceptHistory_t h;
		h.timeSpoken = gpGlobals->curtime;
		idx = m_ConceptHistories.Insert( koncept, h );
	}
	...
	if ( bCallback )
		GetSink()->OnSpokeConcept( koncept, response );
}

//-------------------------------------

void CAI_Expresser::ClearSpokeConcept( AIConcept_t koncept )
{
	m_ConceptHistories.Remove( koncept );
}
