...
public:
	...
	virtual void OnSpokeConcept( CBaseCombatCharacter *who, AIConcept_t koncept, AI_Response *response );	// when an Actor speaks a concept
...
inline void INextBotEventResponder::OnSpokeConcept( CBaseCombatCharacter *who, AIConcept_t koncept, AI_Response *response )
{
	for ( INextBotEventResponder *sub = FirstContainedResponder(); sub; sub = NextContainedResponder( sub ) )
	{
		sub->OnSpokeConcept( who, koncept, response );
	}	
}
