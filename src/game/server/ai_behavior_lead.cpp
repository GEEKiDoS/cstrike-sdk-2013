...
bool CAI_LeadBehavior::Speak( AIConcept_t koncept )
{
	CAI_Expresser *pExpresser = GetOuter()->GetExpresser();
	if ( !pExpresser )
		return false;
	...
	bool bNag = ( FStrEq(koncept,TLK_LEAD_COMINGBACK) || FStrEq(koncept, TLK_LEAD_CATCHUP) || FStrEq(koncept, TLK_LEAD_RETRIEVE) );
	...
	if ( hl2_episodic.GetBool() )
	{
		...
		if ( pAlly )
 			return pAlly->SpeakIfAllowed( koncept, GetConceptModifiers( koncept ) );
	}
	...
	if ( pExpresser->Speak( koncept, GetConceptModifiers( koncept ) ) )
	{
