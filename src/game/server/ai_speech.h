...
{
public:
	virtual void OnSpokeConcept( AIConcept_t koncept, AI_Response *response )	{};
...
public:
	...
	bool Speak( AIConcept_t koncept, const char *modifiers = NULL, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL );

	// These two methods allow looking up a response and dispatching it to be two different steps
	bool SpeakFindResponse( AI_Response &response, AIConcept_t koncept, const char *modifiers = NULL );
	bool SpeakDispatchResponse( AIConcept_t koncept, AI_Response &response, IRecipientFilter *filter = NULL );
	...
	bool CanSpeakConcept( AIConcept_t koncept);
	bool SpokeConcept( AIConcept_t koncept);
	float GetTimeSpokeConcept( AIConcept_t koncept); // returns -1 if never
	void SetSpokeConcept( AIConcept_t koncept, AI_Response *response, bool bCallback = true );
	void ClearSpokeConcept( AIConcept_t koncept);
...
public:
	...
	virtual bool 	Speak( AIConcept_t koncept, const char *modifiers = NULL, char *pszOutResponseChosen = NULL, size_t bufsize = 0, IRecipientFilter *filter = NULL );

	// These two methods allow looking up a response and dispatching it to be two different steps
	bool			SpeakFindResponse( AI_Response& response, AIConcept_t koncept, const char *modifiers = NULL );
	bool 			SpeakDispatchResponse( AIConcept_t koncept, AI_Response& response );
	virtual void	PostSpeakDispatchResponse( AIConcept_t koncept, AI_Response& response ) { return; }
	float 			GetResponseDuration( AI_Response& response );
	...
	void SetSpokeConcept( AIConcept_t koncept, AI_Response *response, bool bCallback = true ) 		{ this->GetExpresser()->SetSpokeConcept( koncept, response, bCallback ); }
	float GetTimeSpokeConcept( AIConcept_t koncept)												{ return this->GetExpresser()->GetTimeSpokeConcept( koncept ); }
	bool SpokeConcept( AIConcept_t koncept )														{ return this->GetExpresser()->SpokeConcept( koncept ); }
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::Speak( AIConcept_t koncept, const char *modifiers /*= NULL*/, char *pszOutResponseChosen /*=NULL*/, size_t bufsize /* = 0 */, IRecipientFilter *filter /* = NULL */ )
{
	AssertOnce( this->GetExpresser()->GetOuter() == this );
	return this->GetExpresser()->Speak( koncept, modifiers, pszOutResponseChosen, bufsize, filter ); 
}
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::SpeakFindResponse( AI_Response& response, AIConcept_t koncept, const char *modifiers /*= NULL*/ )
{
	return this->GetExpresser()->SpeakFindResponse( response, koncept, modifiers );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
template <class BASE_NPC>
inline bool CAI_ExpresserHost<BASE_NPC>::SpeakDispatchResponse( AIConcept_t koncept, AI_Response& response )
{
	if ( this->GetExpresser()->SpeakDispatchResponse(koncept, response ) )
	{
		PostSpeakDispatchResponse( koncept, response );
		return true;
	}
